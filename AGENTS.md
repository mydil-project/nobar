...
- Autoplay: semua `play()` lewat `tryPlay()` — gagal bersuara → retry `muted` → jika masih gagal tampilkan overlay `#hostPlayOverlay`/`#userPlayOverlay` ("Klik untuk memutar"). Gesture pertama (`pointerdown`/`keydown`) via `unlockAudioOnGesture()` unmute jika sempat auto-mute.
...
- Breakpoints: `≤1000px` (sidebar 300px, hide `.time`), `≤900px` (stack column, video `aspect-ratio 16/9`), `≤560px` (phone: header compact, movie-info wrap, form stacked, touch targets ≥40px, playlist item 2-row), `@media (hover: none)` (chat delete always visible), `max-height: 480px` + `≤900px` (landscape: shorter chat/panels).
