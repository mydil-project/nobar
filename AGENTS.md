...
- Breakpoints (in `mobile.css`): `≤1000px` (sidebar 300px, hide `.time`), `≤900px` (stack column, video `aspect-ratio 16/9`, `html/body height: auto` + `padding-bottom: 40vh` + `.app { overflow: visible }` agar scroll saat keyboard terbuka, `#fsToast` top), `≤560px` (phone: header compact, movie-info wrap, form stacked, touch targets ≥40px, playlist item 2-row), `@media (hover: none)` (chat delete always visible), `max-height: 480px` + `≤900px` (landscape: shorter chat/panels).
