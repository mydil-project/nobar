...
- Mobile keyboard: `bindChatInputFocus()` dipanggil **di top-level module** (bukan di `initAll`) — focus/wrapper handler siap sebelum login. Wrapper `.chat-input` `pointerdown` → `hideAllToasts()`; target non-input → `preventDefault`; **selalu** `chatInput.focus()` (jangan rely native focus — Android bisa menelan). `chatInput` `click` juga → `focus()`. `focus` → `hideAllToasts()` + `setTimeout` 300ms → `scrollIntoView({ block: 'nearest', behavior: 'smooth' })` agar input naik di atas keyboard. Viewport meta: `interactive-widget=resizes-content` (Chrome Android resize layout saat keyboard buka). Di `≤900px`: `html/body { height: auto }` + `body { padding-bottom: 40vh }` (scroll room bawah — input di ujung dokumen, tanpa padding browser tidak bisa scroll input ke atas keyboard) + `scroll-margin-bottom: 240px`. Tap `.chat` → `hideAllToasts()`. Toast `#fsToast` di `≤900px` juga dipindah ke atas. Viewer: `initChat()` dipanggil **sebelum** `await get(settings/syncMode)` agar listener chat/counter tidak tertahan network. `initChat.done` guard anti double-bind.
- Chat message count `#chatMsgCount`: hitung dari DOM (`chatMessages.querySelectorAll('.message').length`), bukan hanya `chatEls.size` — cegah angka `0` saat pesan tampil. Script di-load dengan `?v=kb1` untuk bust cache module di mobile.
